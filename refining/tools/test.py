...
def parse_config():
    ...
    cfg.DATA_CONFIG.save_to_file = args.save_to_file
...
def main():
    ...
    root_test = os.path.join(cfg.DATA_CONFIG['DATA_PATH'], cfg.DATA_CONFIG['DATA_SPLIT']['test'], cfg.CLASS_NAMES[0])
    test_seqs_list = sorted([os.path.join(root_test, seq) for seq in os.listdir(root_test)])

    # --- 创建全局 logger（只创建一次） ---
    log_file = Path(cfg.ROOT_DIR) / 'output' / cfg.EXP_GROUP_PATH / cfg.TAG / args.extra_tag / \
    ...
    for idx, test_seq in enumerate(test_seqs_list):
        dir_name_cur = test_seq.split("_")[-1].replace(".pkl", "")
        output_dir = cfg.ROOT_DIR / 'output' / cfg.EXP_GROUP_PATH / cfg.TAG / args.extra_tag / dir_name_cur
        ...
        logger.info(f"No. {idx} Sequence: {dir_name_cur} have finished!")
