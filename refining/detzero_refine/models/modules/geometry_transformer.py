...
class GeometryTransformer(nn.Module):
    ...
    def assign_targets(self, data_dict):
        ...
        for i in range(query_num):
            ...
            # res_dict['geometry_cls'].shape = (batch_size, )
            # res_dict['geometry_reg'].shape = (batch_size, 9)
            geo_cls.append(res_dict['geometry_cls'])
            geo_reg.append(res_dict['geometry_reg'])

        targets_dict = {
            'geometry_cls': torch.stack(geo_cls, dim=1),  # (batch_size, 3)
            'geometry_reg': torch.stack(geo_reg, dim=1)   # (batch_size, 3, 9)
        }
        return targets_dict
    ...
    def forward(self, data_dict):
        """
        data_dict.keys() = [
            'sequence_name', 'frame', 'obj_id', 'obj_cls', 
            'geo_query_num', 'geo_query_boxes', 'geo_query_points', 'geo_memory_points', 
            'geo_trajectory', 'geo_score', 'gt_geo_query_boxes', 'gt_geo_trajectory', 
            'pose', 'state', 'matched', 'matched_tracklet', 'batch_size', 'cur_epoch', 
            'accumulated_iter', 'query', 'memory'
            ]
        data_dict['geo_memory_points'].shape = (128, 4096, 11)
        data_dict['geo_query_boxes/gt_geo_query_boxes'].shape = (128, 3, 7)
        data_dict['geo_query_points'].shape = (128, 4096, 11)
        data_dict['geo_memory_points'].shape = (128, 4096, 11)

        data_dict['geo_trajectory/gt_geo_trajectory'].shape = (128, N, 7)
        data_dict['geo_score'].shape = (128, N)

        data_dict['query'].shape = (128, 256, 3)
        data_dict['memory'].shape = (128, 256, 4096)
        """
        ...
        """
        # preds_dict.keys()=['geometry_cls', 'geometry_reg']
        # preds_dict['geometry_cls'].shape=(1, 128, 3, 3)
        # preds_dict['geometry_reg'].shape=(1, 128, 9, 3)
        """
    ...
    def get_loss(self, tb_dict=None):
        layer_num, bs, max_num, _ = self.preds_dict["geometry_reg"].size()  # (1, 128, 3, 9)
