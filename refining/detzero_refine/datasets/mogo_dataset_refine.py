...
class MogoDataset_evalrefine(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # self.gt_paths_root = "/rss/RALG/data/3.0_pro/Inter+Road/label/shanghai_road_2/p2_lx/train_sh_3d_road_2_20250531_5000_lx/result"
        self.gt_paths_root = "/rss/yuanqingwen/refine_data/fusion/train_sh_3d_road_2_20250531_5000_lx/result"
    ...
    def load_annotations(self, sequence_path):
        ...
        for scene_path in scenes_path:
            ...
            timestamps = sorted(os.listdir(os.path.join(scene_path, "grm_fusion_0923_3w5")))
            tracking_res = sorted([os.path.join(scene_path, "tracking_fusion", item) for item in timestamps])
            pred_res = sorted([os.path.join(scene_path, "pred_lmy", item) for item in timestamps])
            grm_res = sorted([os.path.join(scene_path, "grm_fusion_0923_3w5", item) for item in timestamps])
            grm_prm_res = sorted([os.path.join(scene_path, "combine_fusion_trainxyz_0923_3w5", item) for item in timestamps])
    ...
    def load_pred_txt(self, sample_token, file_path, refine=False, pred=False, radians=False):
        """
        """
        ...
        if not pred:
            scene = file_path.split("/")[-3]
            timestamp = file_path.split("/")[-1]
            drop_file_path = os.path.join("/rss/yuanqingwen/refine_data/fusion/offline_tracked/train_sh_3d_road_2_20250531_5000_lx", scene, "droped", timestamp)
            if os.path.exists(drop_file_path):
    ...
    def parse_ann_info(self, tracking_res, pred_res, grm_res, grm_prm_res, gt_paths):
        ...
        for i in tqdm(range(0, len(tracking_res), self.load_interval)):
            ...
            sample["token"] = token
            sample["tracking_bboxes"] = self.load_pred_txt(token, tracking_res[i], refine=False, pred=False, radians=False)
            sample["detection_bboxes"] = self.load_pred_txt(token, pred_res[i], refine=False, pred=True, radians=False)
            sample["grm_bboxes"] = self.load_pred_txt(token, grm_res[i], refine=True, pred=False, radians=False)
            sample["grm_prm_bboxes"] = self.load_pred_txt(token, grm_prm_res[i], refine=True, pred=False, radians=False)
    ...
    def _main_iou(self, gt_boxes, pred_boxes):
        ...
        mean_ap_03 = mean_over_classes(0.3)


        # 也构造便于直接打印的结构
        AP_at_03 = {c: float(label_aps[c].get("0.3", 0.0)) for c in class_names}
        ...
        metrics = {
            ...
            "mean_ap_0.3": float(mean_ap_03),
            "AP@0.5": AP_at_05,
            "AP@0.3": AP_at_03,
            "mAP@0.5": float(mean_ap_05),
            "mAP@0.3": float(mean_ap_03),
        }
        ...
        if self.verbose:
            print(
                f"[IoU Eval] AP@0.5(mAP)={mean_ap_05:.4f}, AP@0.3(mAP)={mean_ap_03:.4f}; saved to: {out_json}"
            )
    ...
    def _print_console_summary(self, metrics: Dict[str, Any]) -> None:
        ...
        mAP03 = float(metrics.get("mAP@0.3", metrics.get("mean_ap_0.3", 0.0)))

        AP05 = metrics.get("AP@0.5", None)
        AP03 = metrics.get("AP@0.3", None)

        # 兜底：从 label_aps 里还原
        if AP05 is None or AP03 is None:
            AP05, AP03 = {}, {}
            label_aps = metrics.get("label_aps", {})
            for cls, d in label_aps.items():
                AP05[cls] = float(d.get("0.5", 0.0))
                AP03[cls] = float(d.get("0.3", 0.0))
        ...
        # 打印 IoU=0.7
        print("\nResults at IoU = 0.3")
        print(f"mAP@0.3: {mAP03:.4f}")
        print(f"{'Object Class':<16} {'AP@0.3':>8}")
        for cls in sorted(AP03.keys()):
            print(f"{cls:<16} {AP03[cls]:>8.3f}")
    


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='arg parser')
    parser.add_argument('--data-root', type=str, default='/rss/yuanqingwen/data/test_4D_label/origin/model_res/eval_res',
