...
from detzero_refine.utils.data_utils import (sample_points,
                                             ...
                                             init_coords_transform_camera, 
                                             box_coords_transform)

  

class CameraPositionDataset(DatasetTemplate):
    ...
    def extract_track_feature(self, data_info):
        ...
        matched = np.ones_like(data_info['sample_idx'], dtype=bool)
        mth_tk = data_info['matched_tracklet']
        mth_tk = np.zeros_like(data_info['sample_idx'], dtype=bool)

        state = data_info['state']
        traj_gt_all = data_info['gt_boxes_global'][:, :7].astype(np.float32)
        ...
        init_box, traj, traj_gt = init_coords_transform_camera(
            init_box, traj, traj_gt)
        ...
        for i in range(len(pts)):
            pts_sa = sample_points(pts[i].reshape(1, -1), sample_num=self.query_pts_num)
            query_pts.append(pts_sa)

            pts_sa = sample_points(pts[i].reshape(1, -1), sample_num=self.memory_pts_num)
            traj_pts.append(pts_sa)

        query_pts = sample_points(pts, sample_num=self.query_pts_num)
        traj_pts = sample_points(pts, sample_num=self.memory_pts_num)
        
        local_pts_data = query_pts
        global_pts_data = traj_pts

        # pad the box
        if box_num <= self.query_num:
            # padding
            ...
            zeros = np.zeros((self.query_num-box_num, 7), dtype=np.float32)
            ...
            padding_mask = np.concatenate(
                (np.zeros(box_num), np.ones(self.query_num-box_num)), axis=0
            )

        else:
            # traj长度 > query_num, 从 traj 中截取连续 query_num 段
            start_idx = 0
            if self.training:
                # 随机选择起点，保证截取长度为 query_num
                start_idx = np.random.randint(0, box_num - self.query_num + 1)
            # 截取连续片段
            end_idx = start_idx + self.query_num

            local_pts_data = local_pts_data[start_idx:end_idx]
            global_pts_data = global_pts_data[start_idx:end_idx]
            traj_gt = traj_gt[start_idx:end_idx]
            traj = traj[start_idx:end_idx]
            score = score[start_idx:end_idx]
            frm_id = frm_id[start_idx:end_idx]

            box_num = self.query_num
            padding_mask = np.zeros(self.query_num)

        # ================================================
        nums_pose = len(traj)
        pose = [np.eye(4, dtype=np.float32) for _ in range(nums_pose)]
        obj_info = {
