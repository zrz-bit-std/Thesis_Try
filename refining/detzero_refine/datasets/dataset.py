...
class DatasetTemplate(torch.utils.data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, test_seq=None, training=True, root_path=None, logger=None):
        super().__init__()
        ...
        self.class_map = {
            ...
            1: 'Vehicle', 2: 'Pedestrian', 3: 'Cyclist', 4: 'Truck'
        }
        self.box_num = 0
        self.workers_num = 8
    ...
    def load_track_infos(self):
        ...
        if not self.training:
            seq_ids = [self.test_seq]
        else:
            for cls_name in self.class_names:
                data_path = os.path.join(self.root_path, self.split, cls_name)
                file_names = os.listdir(data_path)
                for file in file_names:
                    seq_ids.append(os.path.join(data_path, file))
    ...
    def load_infos_worker(self, seq_name):
        ...
        for obj_id in obj_ids:
            ...
            obj_info['name'] = CLASSES_NAMES[obj_info['name']]
            ...
            try:
                if key in ['obj_cls', 'geo_memory_points',
                    ...
                    for i, box in enumerate(val):
                        box = np.array(box, dtype=np.float32)   # ✅ 保证 float32
