...
from BEVFUSION.tools.utils import poolprocess
...
if not CLEAN_MODE:
    ...
    # 设置0.05与检测分数相同
    class_score_thresholds_distancewise = {
        "car": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "truck": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "bus": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "bike":{(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "bicycle": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "person": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "motor": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "motorcycle": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "NotUsed": {(0, 60): 0.9, (60, 80): 0.9, (80, 1000): 0.05},
        "rider":  {(0, 60): 0.3, (60, 80): 0.05, (80, 1000): 0.05},
        "pedestrain": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
        "pedestrian": {(0, 60): 0.05, (60, 80): 0.05, (80, 1000): 0.05},
    }
...
def worker(args):
    (
        ...
        mode,
        use_fisheye,
        vis_compare_save_root,
    ) = args

    frame_tracted_name = os.path.join(clip_tracked_dir,frame_name)
    frame_merged_name = os.path.join(clip_merged_dir,frame_name)

    dataset_name, clip_name = frame_tracted_name.split("offline_tracked/")[-1].split("/")[:2]
    
    track_save_image_name = os.path.join(
        vis_compare_save_root,
        dataset_name,
        clip_name,
        "track_result",
        frame_name.replace('.txt','.jpg'),
    )
    os.makedirs(os.path.dirname(track_save_image_name), exist_ok=True)
    det_save_image_name = track_save_image_name.replace(
        "track_result", "det_result"
    )
    os.makedirs(os.path.dirname(det_save_image_name), exist_ok=True)

    if not os.path.exists(track_save_image_name) or True:
        boxes_label_tracked = []
        with open(frame_tracted_name,'r') as f:
            for line_ in f.readlines():
                ...
                # if confidence < class_score_thresholds_distancewise[label_name][cur_dist_interval]:  # 2号路口之前生产数据存错了score
                #     continue
        ...
        draw_all_bboxes(
            ...
            image_save_size,
            color_bar,
            track_save_image_name,
            pcr,
            road_id,
        )
    
    if not os.path.exists(det_save_image_name) or True:
        # 读取模型预测结果
        model_res_dir = os.path.join(frame_tracted_name.split("model_res/")[0], "model_res")
        dataset_name, clip_name = frame_tracted_name.split("offline_tracked/")[-1].split("/")[:2]
        model_pred_name = os.path.join(
            model_res_dir,
            f"bevpro/{dataset_name}/model_pred",
            os.path.basename(frame_tracted_name),
        )
        assert os.path.exists(model_pred_name)

        boxes_label_tracked = []
        with open(model_pred_name, "r") as f:
            for line in f.readlines():
                content_list = line.strip().split('\t')
                h, w, l, x, y, z, yaw, confidence = [float(i) for i in content_list[1:]]
                yaw = math.radians(yaw)
                label_name = name_combile_list[name_old_dict[content_list[0]]]
...
def get_pick_data_mp(
    ...
    if len(clip_list) > max_clips:
        indices = np.linspace(0, len(clip_list) - 1, max_clips, dtype=int)
        clip_list = [clip_list[i] for i in indices]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vis_compare_save_root", type=str, required=True)
