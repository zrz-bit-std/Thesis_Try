...
def read_pred_txt(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            # 检查标签是否在映射中
            if vals[0] not in class_name2refine_name:
                print(f"Warning: Unknown label '{vals[0]}' in file {file_path}")
                continue
                
            class_name = class_name2refine_name[vals[0]]  # 获取映射后的类别名，如'person'->'Pedestrian'
