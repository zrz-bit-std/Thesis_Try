...
def generate_labels(frame, filter_empty_obj=True):
    ...
    annotations = common_utils.drop_info_with_name(annotations, name='Sign')  # 过滤掉Sign类别
...
def convert_range_image_to_point_cloud(frame, range_images, camera_projections, range_image_top_pose, ri_index=(0, 1)):
    ...
    # [H, W, 6] - handle dict vs proto for top lidar pose; allow None if missing
    range_image_top_pose_tensor = None
    top_pose = range_image_top_pose
    if isinstance(range_image_top_pose, dict):
        # Waymo API typically returns a dict keyed by LaserName; we need TOP
        top_pose = range_image_top_pose.get(dataset_pb2.LaserName.TOP, None)
    if top_pose is not None:
        if hasattr(top_pose, 'data') and hasattr(top_pose, 'shape'):
            range_image_top_pose_tensor = tf.reshape(
                tf.convert_to_tensor(top_pose.data), top_pose.shape.dims
            )
        else:
            # Fallback if provided as array-like
            range_image_top_pose_tensor = tf.convert_to_tensor(top_pose)
            range_image_top_pose_tensor = tf.reshape(range_image_top_pose_tensor, [-1, -1, 6])
    if range_image_top_pose_tensor is not None:
        # [H, W, 3, 3]
    ...
    for c in calibrations:
        ...
        for cur_ri_index in ri_index:
            ...
            if c.name == dataset_pb2.LaserName.TOP and range_image_top_pose_tensor is not None:
...
def process_single_sequence_and_save(sequence_file: str, save_path: Path, has_label:bool=True, filter_empty_boxes:bool=False,
    ...
    # Normalize to a valid tfrecord path, prefer "_with_camera_labels" version if available
    if sequence_file.endswith('_with_camera_labels.tfrecord'):
        sequence_file_tfrecord = sequence_file
    elif sequence_file.endswith('.tfrecord'):
        sequence_file_tfrecord = sequence_file[:-9] + '_with_camera_labels.tfrecord'
    else:
        sequence_file_tfrecord = sequence_file + '_with_camera_labels.tfrecord'

    if not os.path.exists(sequence_file_tfrecord):
        alt_file = sequence_file_tfrecord.replace('_with_camera_labels', '')
        if os.path.exists(alt_file):
            sequence_file_tfrecord = alt_file
        else:
            print('TFRecord not exists: %s' % sequence_file_tfrecord)
            return []
...
def process_single_tfrecord_multiprocessing(sequence_file:str, has_label:bool=True, filter_empty_boxes:bool=False, pool:multiprocessing.Pool=None):
    ...
    for cnt, data in enumerate(dataset):
        ...
        if has_label:
            annotations = generate_labels(frame, filter_empty_obj=filter_empty_boxes)
...
def save_data_worker(frame):
    res = frame_utils.parse_range_image_and_camera_projection(frame)
    # breakpoint()
    # Support different Waymo Open Dataset versions
    if isinstance(res, (list, tuple)):
        if len(res) >= 3:
            range_images, camera_projections, range_image_top_pose = res[:3]
        else:
            raise ValueError('Unexpected return from parse_range_image_and_camera_projection: length < 3')
    else:
        raise ValueError('Unexpected return type from parse_range_image_and_camera_projection')
    # breakpoint()
