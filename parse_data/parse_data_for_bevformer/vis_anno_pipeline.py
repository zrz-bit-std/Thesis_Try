...
from parse_calib import *
from natsort import natsorted
from nms import process_labels
import config
import sys
...
def generate_txt_from_common_filenames(root_dir, output_txt, dirs=None):
    print('正在从图像文件夹数据中查找交集：')
    # 你的5个目录
    if dirs is None:
        dirs = [
            'camera_0_0', 
...
def copy_image(data_root, data_dir, img_dir, filter_img_list, dst_root_dir, road_info):
    try:
        img_dir_path = os.path.join(data_root, data_dir, img_dir)
        tmp_dst_dir = os.path.join(dst_root_dir, img_dir)
        os.makedirs(tmp_dst_dir, exist_ok=True)
        # img_name = os.listdir(img_dir_path)[0]
        target_img_name = list(set(filter_img_list) & set(os.listdir(img_dir_path)))
        for img_name in target_img_name:
            ...
            if img is None:
                print(f"[Error] Invalid or corrupted image: {img_path}")
                return False
...
def copy_from_dan(data_root, dan_dst_img_dir, img_dirs, road_info):
    ...
    for data_dir in tqdm(data_dirs):
        ...
        if len(os.listdir(os.path.join(data_dir_path, img_dirs[0])))<=10:
            continue
        img_list_tmp = os.listdir(os.path.join(data_dir_path, img_dirs[0]))[10:]
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
        futures = [executor.submit(copy_image, data_root, data_dir, img_dir, filter_img_list, dan_dst_img_dir, road_info) for data_dir, img_dir in tasks]
...
def vis_anno_pipeline(config):
    ...
    # =================== 1. 拷贝 adp格式 图像数据 5s间隔 ====================
    print("1. 拷贝 adp格式 图像数据 5s间隔")
    print(f"Copying file from {data_root} to {dan_dst_img_dir}")
    data_dirs = sorted(os.listdir(data_root))
    ...
    generate_txt_from_common_filenames(dan_dst_img_dir, txt_path)
    ...
    cmd = f"cd /adga/lushiyong/bevformer_road_8cam && python tools/test/4d_inference_pipeline.py --config {CONFIG_PATH} --checkpoint {CHECKPOINT_PATH} --save-path {TXT_SAVE_PATH} --data-root {DATA_ROOT} --ann-file {PKL_PATH} && cd - && cp -r {TXT_SAVE_PATH}/3d_url {DST_DIR}/infer_result"
    print(cmd)
    # # 捕获输出
    process = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1, errors="replace")
    for line in process.stdout:
